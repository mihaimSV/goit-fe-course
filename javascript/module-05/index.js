/*
Используя массив (users) объектов пользователей, напишите функции которые с помощью функциональных методов массивов (никаких for, splice и т.д.) выполняют указанные операции.
*/
const users = [{
...
const getTotalBalance = arr => arr.reduce((accum, item) => accum += item.balance, 0);
console.log(getTotalBalance(users)); // 20916

/*Массив имен всех пользователей у которых есть друг с указанным именем */
const getUsersByFriend = (arr, name) => arr.filter(item => item.friends.some(frnd => frnd === name)).map(obj => obj.name);
...
const getAllSkills = arr => {
    const arrSkillsList = arr.reduce((accArr, obj) => (obj.skills.length > 0) ? accArr.concat(obj.skills) : accArr, []); // reduce по списку объектов собрал все skills в один длинный массив
    const objSkillsList = arrSkillsList.reduce((accObj, skill) => { // reduce по "длинному" списку создает объект с уникальными ключами
        accObj[skill] = true;
        return accObj;
    }, {});
    return Object.keys(objSkillsList).sort();
};
...
const getUserNamesSortedByFriendsCount = arr => {
    ...
    arrUsers.sort((a, b) => a.friends.length - b.friends.length);
    return arrUsers.map(item => item.name);
};
